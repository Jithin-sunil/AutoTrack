...
<html>
    ...
    font-weight: 100;
    background: #111;
    color: #eee; /* Light grey for better contrast on dark background */
}

h1 {
    font-weight: 100;
    color: #fff; /* White for more emphasis */
}

label {
    color: #ddd; /* Softer grey for labels */
}
            .shopping-cart {
                margin-top: -45px;
            }
        ...
        <div class="shopping-cart" style="margin-top: 50px">            <?php                
            ...
            $res = $con->query($sel);
                while ($row=$res->fetch_assoc()) {
                    $selPr = "select * from tbl_spareparts where spareparts_id ='" .$row["spareparts_id"]. "'";
                    $respr = $con->query($selPr);
                    if ($rowpr=$respr->fetch_assoc()) {
                        $selstock = "select sum(stock_quantity) as stock from tbl_stock where spareparts_id ='".$rowpr["spareparts_id"]."'";
						$selstock1="select sum(cart_quantity) as quantity from tbl_cart where spareparts_id ='".$rowpr["spareparts_id"]."' and cart_status >'0'";
            ...
            <div class="product">
                ...
                <div class="product-details">
                    <div class="product-title"><?php echo $rowpr["spareparts_partsname"] ?></div>
                    <p class="product-description">
                    <!-- <?php echo $rowpr["product_details"] ?> -->
                    </p>
                </div>
                <div class="product-price "	><?php echo $rowpr["spareparts_price"] ?></div>
