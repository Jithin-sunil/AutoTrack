...
include('Header.php');
// session_start();
$selcar="select * from tbl_cardetails u inner join tbl_model p on u.model_id = p.model_id inner join tbl_brand w on p.brand_id = w.brand_id inner join tbl_transmissiontype d on u.transmissiontype_id = d.transmissiontype_id  inner join tbl_drivetype e on u.drivetype_id = e.drivetype_id  inner join tbl_fueltype n on u.fueltype_id = n.fueltype_id inner join tbl_emissionnorms m on u.emissionnorms_id = m.emissionnorms_id inner join tbl_steeringtype s on u.steeringtype_id = s.steeringtype_id inner join tbl_braketype v on u.frontbraketype_id = v.braketype_id inner join tbl_braketype z on u.rearbraketype_id = z.braketype_id  where u.cardetails_id=".$_GET['CarID'];
    $car=$con->query($selcar);
	$data=$car->fetch_assoc();
    if (isset($_POST['btntrial'])) 
    {
        header("Location: ViewShowRoom.php?bid=" . $data['brand_id'] . "&carid=" . $_GET['CarID'] . "&bikeid=0");
    }
?>
<!DOCTYPE html>
<html lang="en">
       ...
       <li><a href="#" onclick="cardetails('<?php echo $value['model_id'] ?>')" ><?php echo $value['model_name']?></a></li>
            ...
            <section class="vehicle-details">
                ...
                <div class="details-section">
                    ...
                    <div class="detail-item">
                        <h3>Mileage:</h3>
                        <h5><?php echo $data['cardetails_mileage'] ?> Kmpl</h5>
                    </div>
                    <div class="detail-item">
                        <h3>Fuel Tank Capacity:</h3>
                        <h5><?php echo $data['cardetails_fuelcapacity'] ?> Liter</h5>
                    </div>
                    <div class="detail-item">
                        <h3>Topspeed:</h3>
                        <h5><?php echo $data['cardetails_topspeed'] ?> Km/H</h5>
                    </div>
                    <div class="detail-item">
                        <h3>steering type:</h3>
                    ...
                    <a href="ViewShowRoom.php?bid=<?php echo ($data['brand_id']); ?>&carid=<?php echo ($data['cardetails_id']); ?>&bikeid=0">Trial Booking</a>
...
<script>
function cardetails(cid) {
	$.ajax({
		url:"../Assets/AjaxPages/AjaxCardetails.php?cid=" + cid,
